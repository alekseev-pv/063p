...
using ErrorCode = EOSDigital.SDK.ErrorCode;
...
namespace WinFormsExample
{
    public partial class MainForm : Form
    {
            ...
            else
            {
                lock (LvLock)
                {
                    if (Evf_Bmp != null)
                    {
                        ...
                        var updBmp = DetectFace(Evf_Bmp);

                        e.Graphics.DrawImage(updBmp, 0, 0, w, h);
                    }
                }
            }
        }
        ...
        static Bitmap DetectFace(Bitmap srcImageBmp)
        {
            var srcImage = srcImageBmp.ToMat();
            ...
            // Cv2.CvtColor(srcImage, srcImage, ColorConversionCodes.BGRA2GRAY);

            // Cv2.InRange(srcImage, new Scalar(0, 100, 0), new Scalar(120, 255, 100), srcImage);
